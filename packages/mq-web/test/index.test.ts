...
describe("hover", () => {
  it("returns null for position with no symbol", async () => {
    const result = await hover(".[]", 1, 1);
    expect(result === null || typeof result === "object").toBe(true);
  });

  it("returns hover info for a known builtin", async () => {
    const result = await hover("upcase()", 1, 1);
    if (result !== null) {
