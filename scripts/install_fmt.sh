...
get_download_url() {
    ...
    echo "https://github.com/$MQ_REPO/releases/download/$version/mq-fmt-${target}${ext}"
}
...
install_mqf() {
    ...
    log "mq-fmt installed successfully to $MQ_BIN_DIR/$binary_name"
}
...
# Verify installation
verify_installation() {
    if [[ -x "$MQ_BIN_DIR/mq-fmt" ]] || [[ -x "$MQ_BIN_DIR/mq-fmt.exe" ]]; then
