...
EOF
    echo -e "${BOLD}${CYAN}     mq Language Server Protocol${NC}"
    echo -e "${BLUE}   LSP implementation for mq query language${NC}"
    echo ""
    echo -e "${PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
}
...
get_download_url() {
    ...
    echo "https://github.com/$MQ_REPO/releases/download/$version/mq-lsp-${target}${ext}"
}
...
verify_checksum() {
    ...
    # Format: <checksum>  mq-<target>/mq-lsp-<target>
...
# Download and install mq-lsp
install_mq_lsp() {
    local version="$1"
    local os="$2"
    local arch="$3"
    local download_url
    local binary_name="mq-lsp"
    local ext=""
    local target=""

    if [[ "$os" == "windows" ]]; then
        ext=".exe"
        binary_name="mq-lsp.exe"
        target="${arch}-pc-windows-msvc"
    elif [[ "$os" == "darwin" ]]; then
        target="${arch}-apple-darwin"
    else
    ...
    log "Downloading mq-lsp $version for $os/$arch..."
    ...
    if ! curl -L --progress-bar "$download_url" -o "$temp_file"; then
        error "Failed to download mq-lsp binary"
    fi

    # Verify checksum
    # Checksum format: mq-<target>/mq-lsp-<target>
    local release_binary_name="mq-${target}/mq-lsp-${target}${ext}"
    ...
    log "mq-lsp installed successfully to $MQ_BIN_DIR/$binary_name"
}
...
update_shell_profile() {
    ...
    if [[ -n "$shell_profile" ]]; then
        ...
        if ! grep -q "$MQ_BIN_DIR" "$shell_profile" 2>/dev/null; then
            echo "" >> "$shell_profile"
            echo "# Added by mq-lsp installer" >> "$shell_profile"
            echo "$path_export" >> "$shell_profile"
            log "Added $MQ_BIN_DIR to PATH in $shell_profile"
        else
            log "$MQ_BIN_DIR already exists in $shell_profile"
        fi
    else
...
# Verify installation
verify_installation() {
    if [[ -x "$MQ_BIN_DIR/mq-lsp" ]] || [[ -x "$MQ_BIN_DIR/mq-lsp.exe" ]]; then
        log "✓ mq-lsp installation verified"
        return 0
    else
        error "mq-lsp installation verification failed"
    fi
}
...
show_post_install() {
    echo ""
    echo -e "${PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${BOLD}${GREEN}✨ mq-lsp installed successfully! ✨${NC}"
    echo -e "${PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    ...
    echo -e "     ${CYAN}mq-lsp --version${NC}"
    echo ""
    echo -e "  ${YELLOW}3.${NC} Configure your editor to use mq-lsp as the language server"
    echo ""
    echo -e "${BOLD}${CYAN}📝 Editor Configuration:${NC}"
    echo ""
    echo -e "  ${GREEN}▶${NC} ${BOLD}VS Code:${NC} Install the mq extension from the marketplace"
    echo -e "  ${GREEN}▶${NC} ${BOLD}Neovim:${NC} Configure with nvim-lspconfig"
    echo ""
...
main() {
    ...
    # Install mq-lsp
    install_mq_lsp "$version" "$os" "$arch"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        --help|-h)
            echo "mq-lsp installation script"
            echo ""
