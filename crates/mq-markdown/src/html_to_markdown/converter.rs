    ...
    for node in &list_element.children {
        if let HtmlNode::Element(li_element) = node {
            if li_element.tag_name == "li" {
                ...
                let li_content_markdown = convert_nodes_to_markdown(&li_element.children, html_input_for_error)?;
            ...
            HtmlNode::Element(element) => {
                ...
                match element.tag_name.as_str() {
                    ...
                    "ul" | "ol" => {
                        ...
                        let list_md = convert_html_list_to_markdown(element, 0, html_input_for_error)?;
                    ...
                    "blockquote" => {
                        ...
                        let inner_markdown = convert_nodes_to_markdown(&element.children, html_input_for_error)?;
                       ...
                       for child_node in &element.children {
                           match child_node {
                               ...
                               HtmlNode::Element(dd_el) if dd_el.tag_name == "dd" => {
                                   let dd_markdown_block = convert_nodes_to_markdown(&dd_el.children, html_input_for_error)?;
                               ...
                               HtmlNode::Comment(_) => {}
                               _ => {
                                   let unexpected_block = convert_nodes_to_markdown(&[child_node.clone()], html_input_for_error)?;
