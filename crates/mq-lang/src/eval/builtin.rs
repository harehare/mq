...
pub fn get_builtin_functions(name: &AstIdentName) -> Option<&'static BuiltinFunction> {
    match name.as_str() {
        "abs" => Some(&ABS),
        "add" => Some(&ADD),
        constants::AND => Some(&AND),
        constants::ARRAY => Some(&ARRAY),
        constants::ATTR => Some(&ATTR),
        "base64" => Some(&BASE64),
        "base64d" => Some(&BASE64D),
        "ceil" => Some(&CEIL),
        "compact" => Some(&COMPACT),
        "decrease_header_level" => Some(&DECREASE_HEADER_LEVEL),
        "del" => Some(&DEL),
        constants::DICT => Some(&DICT),
        constants::DIV => Some(&DIV),
        "downcase" => Some(&DOWNCASE),
        "ends_with" => Some(&ENDS_WITH),
        "entries" => Some(&ENTRIES),
        constants::EQ => Some(&EQ),
        "error" => Some(&ERROR),
        "explode" => Some(&EXPLODE),
        "flatten" => Some(&FLATTEN),
        "floor" => Some(&FLOOR),
        "from_date" => Some(&FROM_DATE),
        constants::GET => Some(&GET),
        constants::GT => Some(&GT),
        constants::GTE => Some(&GTE),
        "get_title" => Some(&GET_TITLE),
        "get_url" => Some(&GET_URL),
        "gsub" => Some(&GSUB),
        "halt" => Some(&HALT),
        "implode" => Some(&IMPLODE),
        "increase_header_level" => Some(&INCREASE_HEADER_LEVEL),
        "index" => Some(&INDEX),
        "insert" => Some(&INSERT),
        "join" => Some(&JOIN),
        "keys" => Some(&KEYS),
        "len" => Some(&LEN),
        constants::LT => Some(&LT),
        constants::LTE => Some(&LTE),
        "match" => Some(&MATCH),
        "max" => Some(&MAX),
        "min" => Some(&MIN),
        constants::MOD => Some(&MOD),
        constants::MUL => Some(&MUL),
        constants::NE => Some(&NE),
        constants::NOT => Some(&NOT),
        "now" => Some(&NOW),
        constants::OR => Some(&OR),
        "pow" => Some(&POW),
        "print" => Some(&PRINT),
        constants::RANGE => Some(&RANGE),
        "repeat" => Some(&REPEAT),
        ...
        "round" => Some(&ROUND),
        "set" => Some(&SET),
        "set_attr" => Some(&SET_ATTR),
        "set_check" => Some(&SET_CHECK),
        "set_code_block_lang" => Some(&SET_CODE_BLOCK_LANG),
        "set_list_ordered" => Some(&SET_LIST_ORDERED),
        "set_ref" => Some(&SET_REF),
        constants::SLICE => Some(&SLICE),
        "sort" => Some(&SORT),
        "_sort_by_impl" => Some(&_SORT_BY_IMPL),
        "split" => Some(&SPLIT),
        "starts_with" => Some(&STARTS_WITH),
        "stderr" => Some(&STDERR),
        constants::SUB => Some(&SUB),
        "to_array" => Some(&TO_ARRAY),
        "to_code" => Some(&TO_CODE),
        "to_code_inline" => Some(&TO_CODE_INLINE),
        "to_date" => Some(&TO_DATE),
        "to_em" => Some(&TO_EM),
        "to_h" => Some(&TO_H),
        "to_hr" => Some(&TO_HR),
        "to_html" => Some(&TO_HTML),
        "to_image" => Some(&TO_IMAGE),
        "to_link" => Some(&TO_LINK),
        "to_markdown_string" => Some(&TO_MARKDOWN_STRING),
        "to_math" => Some(&TO_MATH),
        "to_math_inline" => Some(&TO_MATH_INLINE),
        "to_md_list" => Some(&TO_MD_LIST),
        "to_md_name" => Some(&TO_MD_NAME),
        "to_md_table_row" => Some(&TO_MD_TABLE_ROW),
        "to_md_text" => Some(&TO_MD_TEXT),
        "to_number" => Some(&TO_NUMBER),
        "to_string" => Some(&TO_STRING),
        "to_strong" => Some(&TO_STRONG),
