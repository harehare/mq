...
/// Collects the union of dict keys across an array of dict records, ignoring non-dict elements.
fn record_array_schema(records: &[RuntimeValue]) -> FxHashSet<Ident> {
    records
        .iter()
        .filter_map(|v| match v {
            RuntimeValue::Dict(map) => Some(map.keys().copied()),
            _ => None,
        })
        .flatten()
        .collect()
...
/// Merges two dict records; on key collision, `right`'s value wins.
fn merge_records(left: &BTreeMap<Ident, RuntimeValue>, right: &BTreeMap<Ident, RuntimeValue>) -> RuntimeValue {
    let mut merged = left.clone();
    merged.extend(right.iter().map(|(k, v)| (*k, v.clone())));
    RuntimeValue::Dict(Shared::new(merged))
}

/// Fills a record with `RuntimeValue::None` for every key present in `schema` but absent from the record.
fn fill_missing_keys(record: &BTreeMap<Ident, RuntimeValue>, schema: &FxHashSet<Ident>) -> RuntimeValue {
    let mut filled = record.clone();
    for key in schema {
        filled.entry(*key).or_insert(RuntimeValue::NONE);
    }
    RuntimeValue::Dict(Shared::new(filled))
}

#[mq_macros::mq_fn(name = "join_by", params = Range(4, 5))]
fn join_by_impl(ident: &Ident, _: &RuntimeValue, mut args: Args, _: &SharedEnv) -> Result<RuntimeValue, Error> {
    let (left, right, left_key, right_key, kind) = match args.as_mut_slice() {
        [
            RuntimeValue::Array(left),
            RuntimeValue::Array(right),
            RuntimeValue::String(left_key),
            RuntimeValue::String(right_key),
        ] => (left, right, left_key.as_str(), right_key.as_str(), "inner"),
        [
            RuntimeValue::Array(left),
            RuntimeValue::Array(right),
            RuntimeValue::String(left_key),
        ...
        ] => (left, right, left_key.as_str(), right_key.as_str(), kind.as_str()),
        args => return Err(Error::InvalidTypes(ident.to_string(), args.to_vec())),
    };

    if !matches!(kind, "inner" | "left" | "right" | "full") {
        return Err(Error::Runtime(format!(
            r#"invalid join kind "{kind}", expected one of "inner", "left", "right", "full""#
        )));
    }

    let left_ident = Ident::new(left_key);
    let right_ident = Ident::new(right_key);
    let left_records = runtime_value::array_mut(left);
    let right_records = runtime_value::array_mut(right);

    for record in left_records.iter().chain(right_records.iter()) {
        if !matches!(record, RuntimeValue::Dict(_)) {
            return Err(Error::InvalidTypes(ident.to_string(), vec![record.clone()]));
        }
    }

    let mut right_index: FxHashMap<String, Vec<usize>> = FxHashMap::default();
    for (i, record) in right_records.iter().enumerate() {
        if let RuntimeValue::Dict(map) = record
            && let Some(key_value) = map.get(&right_ident)
            && !matches!(key_value, RuntimeValue::None)
        {
            right_index.entry(key_value.to_string()).or_default().push(i);
        }
    }

    let right_schema = matches!(kind, "left" | "full").then(|| record_array_schema(right_records));
    let left_schema = matches!(kind, "right" | "full").then(|| record_array_schema(left_records));

    let mut matched_right_indices: FxHashSet<usize> = FxHashSet::default();
    let mut out = Vec::new();

    for record in left_records.iter() {
        let RuntimeValue::Dict(left_map) = record else {
            unreachable!("non-dict left records were rejected above")
        };
        let matches = left_map
            .get(&left_ident)
            .filter(|v| !matches!(v, RuntimeValue::None))
            .and_then(|v| right_index.get(&v.to_string()));

        match matches {
            Some(indices) if !indices.is_empty() => {
                for &i in indices {
                    matched_right_indices.insert(i);
                    let RuntimeValue::Dict(right_map) = &right_records[i] else {
                        unreachable!("non-dict right records were rejected above")
                    };
                    out.push(merge_records(left_map, right_map));
                }
            }
            _ if matches!(kind, "left" | "full") => {
                out.push(fill_missing_keys(left_map, right_schema.as_ref().unwrap()));
            }
            _ => {}
        }
    }
    ...
    if matches!(kind, "right" | "full") {
        for (i, record) in right_records.iter().enumerate() {
            ...
            let RuntimeValue::Dict(right_map) = record else {
                unreachable!("non-dict right records were rejected above")
            };
            out.push(fill_missing_keys(right_map, left_schema.as_ref().unwrap()));
        }
    }

    Ok(RuntimeValue::Array(Shared::new(out)))
}
...
world"# }],
    ...
    map.insert(
        SmolStr::new("join_by"),
        BuiltinFunctionDoc {
            description: "Joins two arrays of dict records on `left_key`/`right_key`, merging matched records (right's fields win on collision). `kind` is one of \"inner\" (default), \"left\", \"right\", or \"full\"; unmatched records in outer joins are filled with `None` for the other side's fields.",
            params: &["left", "right", "left_key", "right_key", "kind?"],
        },
    );
    map.insert(
        SmolStr::new("reverse"),
...
mod tests {
    ...
    fn record(entries: &[(&str, RuntimeValue)]) -> RuntimeValue {
        RuntimeValue::Dict(Shared::new(
            entries.iter().map(|(k, v)| (Ident::new(k), v.clone())).collect(),
        ))
    }

    fn call_join_by(args: Vec<RuntimeValue>) -> Result<RuntimeValue, Error> {
        eval_builtin(
            &RuntimeValue::None,
            &Ident::new("join_by"),
            args,
    ...
    #[test]
    fn test_join_by_inner() {
        let left = RuntimeValue::Array(Shared::new(vec![
            record(&[("id", RuntimeValue::Number(1.0.into())), ("name", "alice".into())]),
            record(&[("id", RuntimeValue::Number(2.0.into())), ("name", "bob".into())]),
        ]));
        let right = RuntimeValue::Array(Shared::new(vec![
            record(&[
                ("uid", RuntimeValue::Number(2.0.into())),
                ("age", RuntimeValue::Number(30.0.into())),
            ]),
            record(&[
                ("uid", RuntimeValue::Number(3.0.into())),
                ("age", RuntimeValue::Number(40.0.into())),
            ]),
        ]));

        let result = call_join_by(vec![left, right, "id".into(), "uid".into()]);

        assert_eq!(
            result,
            Ok(RuntimeValue::Array(Shared::new(vec![record(&[
                ("id", RuntimeValue::Number(2.0.into())),
                ("name", "bob".into()),
                ("uid", RuntimeValue::Number(2.0.into())),
                ("age", RuntimeValue::Number(30.0.into())),
            ])])))
    ...
    #[test]
    fn test_join_by_left_fills_missing_with_none() {
        let left = RuntimeValue::Array(Shared::new(vec![
            record(&[("id", RuntimeValue::Number(1.0.into())), ("name", "alice".into())]),
            record(&[("id", RuntimeValue::Number(2.0.into())), ("name", "bob".into())]),
        ]));
        let right = RuntimeValue::Array(Shared::new(vec![record(&[
            ("uid", RuntimeValue::Number(2.0.into())),
            ("age", RuntimeValue::Number(30.0.into())),
        ])]));

        let result = call_join_by(vec![left, right, "id".into(), "uid".into(), "left".into()]);

        assert_eq!(
            result,
            Ok(RuntimeValue::Array(Shared::new(vec![
                record(&[
                    ("id", RuntimeValue::Number(1.0.into())),
                    ("name", "alice".into()),
                    ("uid", RuntimeValue::NONE),
                    ("age", RuntimeValue::NONE),
                ]),
                record(&[
                    ("id", RuntimeValue::Number(2.0.into())),
                    ("name", "bob".into()),
    ...
    #[test]
    fn test_join_by_right_fills_missing_with_none() {
        let left = RuntimeValue::Array(Shared::new(vec![record(&[
            ("id", RuntimeValue::Number(2.0.into())),
            ("name", "bob".into()),
        ])]));
        let right = RuntimeValue::Array(Shared::new(vec![
        ...
        let result = call_join_by(vec![left, right, "id".into(), "uid".into(), "right".into()]);
        ...
        assert_eq!(
            ...
            Ok(RuntimeValue::Array(Shared::new(vec![
                ...
                record(&[
                    ("id", RuntimeValue::NONE),
                    ("name", RuntimeValue::NONE),
                    ("uid", RuntimeValue::Number(3.0.into())),
                    ("age", RuntimeValue::Number(40.0.into())),
                ]),
            ])))
    ...
    #[test]
    fn test_join_by_full_fills_both_sides() {
        let left = RuntimeValue::Array(Shared::new(vec![
            record(&[("id", RuntimeValue::Number(1.0.into())), ("name", "alice".into())]),
            record(&[("id", RuntimeValue::Number(2.0.into())), ("name", "bob".into())]),
        ]));
        ...
        let result = call_join_by(vec![left, right, "id".into(), "uid".into(), "full".into()]);
    ...
    #[test]
    fn test_join_by_many_to_many_cross_product() {
        let left = RuntimeValue::Array(Shared::new(vec![
            record(&[("id", RuntimeValue::Number(1.0.into())), ("l", "l1".into())]),
            record(&[("id", RuntimeValue::Number(1.0.into())), ("l", "l2".into())]),
        ]));
        let right = RuntimeValue::Array(Shared::new(vec![
            record(&[("id", RuntimeValue::Number(1.0.into())), ("r", "r1".into())]),
            record(&[("id", RuntimeValue::Number(1.0.into())), ("r", "r2".into())]),
        ]));

        let result = call_join_by(vec![left, right, "id".into(), "id".into()]);
    ...
    #[test]
    fn test_join_by_field_collision_right_wins() {
        let left = RuntimeValue::Array(Shared::new(vec![record(&[
            ("id", RuntimeValue::Number(1.0.into())),
            ("value", "from_left".into()),
        ])]));
        let right = RuntimeValue::Array(Shared::new(vec![record(&[
            ("id", RuntimeValue::Number(1.0.into())),
            ("value", "from_right".into()),
        ])]));
    ...
    #[test]
    fn test_join_by_none_keys_never_match() {
        let left = RuntimeValue::Array(Shared::new(vec![record(&[
            ("id", RuntimeValue::NONE),
            ("name", "alice".into()),
        ])]));
        ...
        assert_eq!(result, Ok(RuntimeValue::Array(Shared::new(vec![]))));
    }
    ...
    #[case("")]
    #[case("Inner")]
    fn test_join_by_invalid_kind(#[case] kind: &str) {
        let left = RuntimeValue::Array(Shared::new(vec![]));
        let right = RuntimeValue::Array(Shared::new(vec![]));

        let result = call_join_by(vec![left, right, "id".into(), "uid".into(), kind.into()]);

        assert!(matches!(result, Err(Error::Runtime(_))));
    }

    #[test]
    fn test_join_by_non_dict_element_errors() {
        let left = RuntimeValue::Array(Shared::new(vec![RuntimeValue::Number(1.0.into())]));
        let right = RuntimeValue::Array(Shared::new(vec![]));

        let result = call_join_by(vec![left, right, "id".into(), "uid".into()]);

        assert!(matches!(result, Err(Error::InvalidTypes(..))));
    }
