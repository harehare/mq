...
pub struct Hir {
    pub builtin: Builtin,
    pub(crate) module_loader: mdq_lang::ModuleLoader,
...
impl Hir {
    pub fn new() -> Self {
        ...
        Self {
            ...
            sources,
            scopes,
            module_loader: mdq_lang::ModuleLoader::new(None),
            source_scopes,
    ...
    pub fn builtins(&self) -> impl Iterator<Item = &mdq_lang::BuiltinFunctionDoc> {
    ...
    pub fn add_line_of_code(&mut self, source_id: SourceId, scope_id: ScopeId, code: &str) {
        let (nodes, _) = mdq_lang::parse_recovery(code);
    ...
    pub fn add_builtin(&mut self) {
        ...
        let (nodes, _) = mdq_lang::parse_recovery(mdq_lang::ModuleLoader::BUILTIN_FILE);
        ...
        self.builtin.functions.clone().keys().for_each(|name| {
            self.add_symbol(Symbol {
                ...
                kind: SymbolKind::Function(
                    mdq_lang::BUILTIN_FUNCTION_DOC[name]
                        .params
                        .iter()
                ...
                doc: vec![(
                    mdq_lang::Range::default(),
                    mdq_lang::BUILTIN_FUNCTION_DOC[name].description.to_string(),
                )],
                parent: None,
            });
        });
        ...
        self.builtin.selectors.clone().keys().for_each(|name| {
            self.add_symbol(Symbol {
                ...
                doc: vec![(
                    mdq_lang::Range::default(),
                    mdq_lang::BUILTIN_SELECTOR_DOC[name].description.to_string(),
                )],
                parent: None,
            });
        });
    ...
    pub fn add_nodes(&mut self, url: Url, nodes: &[Arc<mdq_lang::CstNode>]) -> (SourceId, ScopeId) {
        self.add_builtin();
        ...
        } = &**node
        {
            self.add_symbol(Symbol {
                ...
                kind: match &node.token.clone().unwrap().kind {
                    mdq_lang::TokenKind::StringLiteral(_) => SymbolKind::String,
                    mdq_lang::TokenKind::NumberLiteral(_) => SymbolKind::Number,
                    mdq_lang::TokenKind::BoolLiteral(_) => SymbolKind::Boolean,
                    mdq_lang::TokenKind::None => SymbolKind::None,
                    _ => unreachable!(),
                },
    ...
    fn add_include_expr(
        ...
        if let mdq_lang::CstNode {
            kind: mdq_lang::CstNodeKind::Include,
            ..
        } = &**node
        {
    ...
    fn add_while_expr(
        ...
        if let mdq_lang::CstNode {
            kind: mdq_lang::CstNodeKind::While,
            ..
        } = &**node
        {
    ...
    fn add_until_expr(
        ...
        if let mdq_lang::CstNode {
            kind: mdq_lang::CstNodeKind::Until,
            ..
        } = &**node
        {
            let symbol_id = self.add_symbol(Symbol {
    ...
    fn add_ident_expr(
        ...
        if let mdq_lang::CstNode {
            kind: mdq_lang::CstNodeKind::Ident,
            ..
        } = &**node
        {
            self.symbols.insert(Symbol {
    ...
    fn add_elif_expr(
        ...
        if let mdq_lang::CstNode {
            kind: mdq_lang::CstNodeKind::Elif,
            ..
        } = &**node
        {
            let symbol_id = self.add_symbol(Symbol {
    ...
    fn add_else_expr(
        ...
        if let mdq_lang::CstNode {
            kind: mdq_lang::CstNodeKind::Else,
            ..
        } = &**node
        {
            let symbol_id = self.add_symbol(Symbol {
    ...
    fn add_call_expr(
        ...
        if let mdq_lang::CstNode {
            kind: mdq_lang::CstNodeKind::Call,
            ..
        } = &**node
        {
            self.add_symbol(Symbol {
            ...
            node.children_without_token().iter().for_each(|child| {
                if matches!(child.kind, mdq_lang::CstNodeKind::Literal) {
    ...
    fn add_foreach_expr(
        ...
        if let mdq_lang::CstNode {
            kind: mdq_lang::CstNodeKind::Foreach,
            ..
        } = &**node
        {
            let symbol_id = self.add_symbol(Symbol {
...
mod tests {
    ...
    #[rstest]
    #[case::let_("let x = 1;", mdq_lang::Position::new(1, 5), "x", SymbolKind::Variable)]
    #[case::def(
        "def foo(): 1",
        mdq_lang::Position::new(1, 6),
        "foo",
        SymbolKind::Function(Vec::new())
    )]
    #[case::if_(
        "if (true): 1 else: 2;",
        mdq_lang::Position::new(1, 1),
        "if",
        SymbolKind::If
    )]
    #[case::while_(
    ...
    #[case::call(
        "def foo():1; foo()",
        mdq_lang::Position::new(1, 15),
        "foo",
        SymbolKind::Function(Vec::new())
    )]
    fn test_find_symbol_in_position(
        #[case] code: &str,
        #[case] pos: mdq_lang::Position,
        #[case] expected_name: &str,
        #[case] expected_kind: SymbolKind,
    ) {
        let mut hir = Hir::new();
