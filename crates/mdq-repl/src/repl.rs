...
impl Validator for MdqLineHelper {
    fn validate(&self, ctx: &mut ValidationContext<'_>) -> Result<ValidationResult, ReadlineError> {
        ...
        if mdq_lang::parse_recovery(input).1.has_errors() {
...
impl Repl {
    pub fn new(input: Vec<mdq_lang::Value>) -> Self {
    ...
    pub fn config_dir() -> Option<std::path::PathBuf> {
        std::env::var_os("MDQ_CONFIG_DIR")
            ...
            .or_else(|| dirs::config_dir().map(|d| d.join("mdq")))
    }
    ...
    pub fn run(&self) -> miette::Result<()> {
        ...
        println!("Welcome to mdq. For help, type :help");
