...
use mdq_lsp::capabilities;
use mdq_lsp::completions;
use mdq_lsp::document_symbol;
use mdq_lsp::goto_definition;
use mdq_lsp::hover;
use mdq_lsp::references;
use mdq_lsp::semantic_tokens;
...
struct Backend {
    ...
    source_map: RwLock<BiMap<String, mdq_hir::SourceId>>,
    error_map: DashMap<String, Vec<(std::string::String, mdq_lang::Range)>>,
    cst_nodes_map: DashMap<String, Vec<Arc<mdq_lang::CstNode>>>,
}
...
impl LanguageServer for Backend {
    ...
    async fn formatting(&self, params: DocumentFormattingParams) -> Result<Option<Vec<TextEdit>>> {
        ...
        let nodes = self
            ...
            .unwrap();
        let formatted_text =
            mdq_formatter::Formatter::new(Some(mdq_formatter::FormatterConfig { indent_width: 2 }))
...
impl Backend {
    async fn on_change(&self, uri: Url, text: String) {
        let (nodes, errors) = mdq_lang::parse_recovery(&text);
...
async fn main() {
    ...
    let (service, socket) = LspService::new(|client| Backend {
        client,
        hir: Arc::new(RwLock::new(mdq_hir::Hir::new())),
